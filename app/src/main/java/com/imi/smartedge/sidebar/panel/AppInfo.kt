...
data class AppInfo(
     ...
     * For custom items, it's the smartedge.custom.<uuid> token stored in [packageName]
     * (NOT the URL/intent body in [intentUri] — that is only the launch payload).
     */
    val identifier: String
        get() = when (type) {
            Type.CUSTOM -> packageName
            Type.ACTIVITY, Type.SHORTCUT -> intentUri ?: packageName
            else -> intentUri ?: packageName
        }
}
